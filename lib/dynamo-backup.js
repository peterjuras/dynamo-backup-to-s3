...
//    var upload = new Uploader(params);
    ...
    self._copyTable(
        ...
        function (err) {
            console.log('Finished table ' + tableName + ' with count: ' + count);
            stream.end();
            if (err) {
                self.emit('error', {
                    table: tableName,
...
DynamoBackup.prototype._copyTable = function (tableName, itemsReceived, callback) {
    ...
    ddb.describeTable({ TableName: tableName }, function (err, { Table }) {
        if (err) {
            return callback(err);
        }
...
DynamoBackup.prototype._streamItemsOld = function fetchItems(tableName, startKey, limit, itemsReceived, callback) {
    var self = this;
    var ddb = new AWS.DynamoDB();
    var params = {
        Limit: limit,
